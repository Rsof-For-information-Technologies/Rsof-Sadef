...
namespace Sadef.Application.Services.Contact
{
    public class ContactService : IContactService
    {
        ...
        public async Task<Response<ContactDto>> CreateContactAsync(CreateContactDto dto)
        {
            ...
            {
                var propertyQueryRepo = _queryRepositoryFactory.QueryRepository<Property>();
                var propertyExists = await propertyQueryRepo
                    .Queryable()
            ...
            var createdContact = await contactQueryRepo.Queryable()
                .Include(c => c.Property)
                    .ThenInclude(p => p.Translations)
                .FirstOrDefaultAsync(c => c.Id == contact.Id);

            var responseDto = _mapper.Map<ContactDto>(createdContact);
